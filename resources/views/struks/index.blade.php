...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 animate-fadeIn">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            {{-- Search Bar --}}
<div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-end">
    <form action="{{ route('struks.index') }}" method="GET" class="relative w-full max-w-xs">
        <input type="text" name="search" value="{{ request('search') }}"
            placeholder="Cari nama toko atau nomor struk..."
            class="pl-10 pr-4 py-2 rounded-lg border border-slate-300 bg-white text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all duration-200" />

        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M21 21l-4.35-4.35m1.25-5.4a6 6 0 11-12 0 6 6 0 0112 0z" />
            </svg>
        </div>
    </form>
</div>
